...
export default function TableLayoutClient({ table, children }: TableLayoutClientProps) {
  ...
  // Check if we're on the data page (full table view)
  const isDataPage = pathname === `/dashboard/tables/${table.id}/data`
  ...
  // Data page needs special layout - full height for table
  if (isDataPage) {
    return (
